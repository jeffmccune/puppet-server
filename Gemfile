...
group :test do
  ...
  if ENV['GEM_SOURCE'] =~ /rubygems\.delivery\.puppetlabs\.net/
    # gem 'sqa-utils', *location_for(ENV['SQA_UTILS_LOCATION'] || '~> 0.11')
    gem 'sqa-utils', *location_for(ENV['SQA_UTILS_LOCATION'] || 'git@github.com:jeffmccune/sqa-utils-gem.git#jeff_sandbox')
  end
  gem 'httparty'
  gem 'uuidtools'
end
