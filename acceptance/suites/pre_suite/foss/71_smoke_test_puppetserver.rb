...
case variant
  when /^(redhat|el|centos)$/
    ...
    on(master, "service puppetserver start",  :acceptable_exit_codes => [0])
    on(master, "service puppetserver status", :acceptable_exit_codes => [0])
    on(master, "service puppetserver stop",   :acceptable_exit_codes => [0])
  else
